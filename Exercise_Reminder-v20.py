...
class ExerciseReminder:
    ...
    @staticmethod
    def _fuzzy_word_match(query_word: str, log_word: str) -> bool:
        if not query_word or not log_word:
            return False
        if query_word == log_word:
            return True
        if query_word in log_word or log_word in query_word:
            return True
        return SequenceMatcher(None, query_word, log_word).ratio() >= 0.75

    def _query_words_from_comment(self, comment_text: str, max_words: int = 3) -> list[str]:
        words = self._tokenize_words(comment_text)
        return words[:max_words]

    def search_prior_log(self, comment_text: str, max_results: int = 10) -> list[str]:
        """Fuzzy, order-independent, capitalization-agnostic search of the log
        for lines whose text matches most of the first 1-3 words typed in the
        comment field. Returns up to `max_results` matching raw log lines,
        most recent first."""
        query_words = self._query_words_from_comment(comment_text)
        if not query_words:
            return []

        required_matches = len(query_words) // 2 + 1  # strict majority
        ...
        try:
            with open(log_path, "r", encoding="utf-8") as f:
                for raw_line in f:
                    ...
                    entry_text = line[20:].strip()
                    if not entry_text or entry_text == "Exercise period closed":
                        continue

                    entry_words = self._tokenize_words(entry_text)
                    if not entry_words:
                        continue

                    matched = 0
                    for qw in query_words:
                        if any(self._fuzzy_word_match(qw, ew) for ew in entry_words):
                            matched += 1

                    if matched >= required_matches:
                        matches.append((entry_time, line))
        except Exception:
            return []

        matches.sort(key=lambda item: item[0], reverse=True)
        return [line for _, line in matches[:max_results]]

    def show_prior_log(self, comment_var: "tk.StringVar", parent: tk.Toplevel | None = None) -> None:
        comment_text = (comment_var.get() or "").strip()
        if not comment_text:
            messagebox.showinfo("Prior Log", "Type a word or two in the Comment field first, then click Prior Log.")
            return

        results = self.search_prior_log(comment_text)

        owner = parent if parent is not None else self.root

        popup = tk.Toplevel(owner)
        popup.title(f"Prior Log matches for \"{comment_text}\"")

        # Size and position the popup next to (to the right of) the owner
        # window so it doesn't get hidden behind an always-on-top parent.
        popup_w, popup_h = 520, 360
        try:
            ...
            x = owner_x + owner_w + 10
            if x + popup_w > screen_w:
                x = max(0, owner_x - popup_w - 10)
            y = owner_y
        except Exception:
            x, y = 100, 100
        ...
        header = ttk.Label(
            popup,
            text=f"Last {len(results)} match(es) for: {comment_text}" if results else f"No prior matches for: {comment_text}",
            font=("Arial", 10, "bold"),
            wraplength=480,
            justify="left",
        )
        header.pack(padx=10, pady=(10, 6), anchor="w")

        list_frame = ttk.Frame(popup)
        list_frame.pack(fill="both", expand=True, padx=10, pady=(0, 10))

        text_widget = tk.Text(list_frame, wrap="word", height=14)
        ...
        if results:
            ...
            text_widget.insert("1.0", "No prior log entries matched those words.")
        text_widget.configure(state="disabled")

        close_btn = ttk.Button(popup, text="Close", command=popup.destroy, width=10)
        close_btn.pack(pady=(0, 10))

        popup.transient(owner)
        # The Exercise Now window is itself always-on-top, so an ordinary
        # Toplevel can end up rendered behind it. Marking the popup
        # topmost too (and lifting it) ensures it appears above/next to
        # the exercise window instead of hiding behind it.
    ...
    def _add_stopwatch_to_window(self, parent: tk.Toplevel, container: ttk.Frame) -> None:
        ...
        prior_log_row = ttk.Frame(sw_frame)
        prior_log_row.pack(fill="x", pady=(10, 0))

        prior_log_btn = ttk.Button(
            prior_log_row,
            text="Prior Log",
            width=12,
            command=lambda: self.show_prior_log(comment_var, parent),
        )
        prior_log_btn.pack(side="right")
